...
REGISTER_OP("FarthestPointSample1")
  .Attr("npoint: int")
  .Attr("randnum: int")
  .Input("inp: float32")
  .Output("out: int32")
  .SetShapeFn([](::tensorflow::shape_inference::InferenceContext* c) {
    ...
    int randnum;
    TF_RETURN_IF_ERROR(c->GetAttr("npoint", &npoint));
    TF_RETURN_IF_ERROR(c->GetAttr("randnum", &randnum));
    ::tensorflow::shape_inference::ShapeHandle output = c->MakeShape({c->Dim(dims1, 0), npoint});
    c->set_output(0, output);
    return Status::OK();
  });
...
void farthestpointsamplingLauncher1(int b,int n,int m,int randnum,const float * inp,float * temp,int * out);
class FarthestPointSample1GpuOp: public OpKernel{
  public:
    explicit FarthestPointSample1GpuOp(OpKernelConstruction* context):OpKernel(context) {
                    OP_REQUIRES_OK(context, context->GetAttr("npoint", &npoint_));
                    OP_REQUIRES_OK(context, context->GetAttr("randnum", &randnum_));
                    OP_REQUIRES(context, npoint_ > 0, errors::InvalidArgument("FarthestPointSample expects positive npoint"));
                }
    void Compute(OpKernelContext * context)override{
      int m = npoint_;
      ...
      auto temp_flat=temp_tensor.flat<float>();
      float * temp=&(temp_flat(0));
      farthestpointsamplingLauncher1(b,n,m,r,inp,temp,out);
    }
    private:
        int npoint_;
        int randnum_;
};
REGISTER_KERNEL_BUILDER(Name("FarthestPointSample1").Device(DEVICE_GPU),FarthestPointSample1GpuOp);
