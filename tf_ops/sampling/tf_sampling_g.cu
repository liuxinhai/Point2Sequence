...
__global__ void farthestpointsamplingKernel1(int b,int n,int m,int randnum,const float * __restrict__ dataset,float * __restrict__ temp,int * __restrict__ idxs){
  if (m<=0)
    return;
  const int BlockSize=512;
  __shared__ float dists[BlockSize];
...
void farthestpointsamplingLauncher1(int b,int n,int m,int randnum,const float * inp,float * temp,int * out){
  farthestpointsamplingKernel1<<<32,512>>>(b,n,m,randnum,inp,temp,out);
}
