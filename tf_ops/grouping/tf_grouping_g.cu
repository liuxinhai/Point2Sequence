...
// input: radius1 (1), radius2 (1), nsample (1), xyz1 (b,n,3), xyz2 (b,m,3)
// output: idx (b,m,nsample), pts_cnt (b,m)
__global__ void query_ball_point1_gpu(int b, int n, int m, float radius1, float radius2, int nsample, const float *xyz1, const float *xyz2, int *idx, int *pts_cnt) {
    int batch_index = blockIdx.x;
    xyz1 += n*3*batch_index;
    xyz2 += m*3*batch_index;
    idx += m*nsample*batch_index;
...
void queryBallPoint1Launcher(int b, int n, int m, float radius1, float radius2, int nsample, const float *xyz1, const float *xyz2, int *idx, int *pts_cnt) {
    query_ball_point1_gpu<<<b,256>>>(b,n,m,radius1,radius2,nsample,xyz1,xyz2,idx,pts_cnt);
    //cudaDeviceSynchronize();
}
